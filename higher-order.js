...
//Code Here
let totalPopulation = populations.reduce(function(pop, num) {
  return pop + num
})
...
let myStrongest = monstersInYourPocket.filter(function(cp, mon) {
  if (monstersInYourPocket[mon].CP > 200) {
    return monstersInYourPocket[mon]
  }
})
...
let orderTotals = orders.map(function(num, order) {
...
let bobsTotal = purchases.reduce(function(accu, num){
  if(num.owner === 'Bob'){
    return accu + num.price
  } else {
    return accu
  }
}, 0)